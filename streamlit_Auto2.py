...
def send_email(name, email, phone, gender_age, car_brand, mileage, additional_info):
    ...
    subject = "(신차구매) 오토커넥트 챗봇에서 보낸 상담 요청 인적사항입니다"
    
    body = f"""
    요청자가 선택한 컨설팅은 신차구매입니다^^
...
# 새로운 메시지 입력
prompt = st.text_input("오토커넥트 챗봇에게 물어보세요!")
if prompt:
    # 사용자의 메시지 스레드에 추가
    ...
    # 메시지를 chat_history에 저장
    for msg in reversed(messages.data):
        role = "나" if msg.role == "user" else "오토커넥트 챗봇"
        st.session_state.chat_history.append((role, msg.content))
...
for role, content in st.session_state.chat_history:
    if role == "나":
        st.markdown(
            ...
            f'{content[0].text.value}</div></div>',
            unsafe_allow_html=True
        )
    else:
        st.markdown(
            ...
            f'<strong>{role}<br>:</strong> {content[0].text.value}</div></div>',
            unsafe_allow_html=True
        )
